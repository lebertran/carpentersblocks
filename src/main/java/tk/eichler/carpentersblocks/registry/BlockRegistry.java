...
import tk.eichler.carpentersblocks.model.CarpentersCoverableModel;
...
public class BlockRegistry extends BaseRegistry {
    ...
    public static BlockRegistry INSTANCE = new BlockRegistry();
    ...
    private static void loadModels(IRegistry<ModelResourceLocation, IBakedModel> modelRegistry) {
        final IBakedModel customModel = new CarpentersCoverableModel();

        for (BaseBlock block : BlockRegistry.ALL_BLOCKS) {
            modelRegistry.putObject(block.getBlockModelLocation(), customModel);
        }
    }
}
